          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
                  ...
                  if (component_i < dim && component_j < dim)
                    {
                      ...
                      cell_mass_matrix(i, j) +=
                        scalar_product(phi_i_u, phi_j_u) * fe_values.JxW(q);
                      cell_matrix(i, j) += (1.0 / time_step) *
                                           scalar_product(phi_i_u, phi_j_u) *
                                           fe_values.JxW(q);
                    }
              ...
              // RHS term from time derivative (using old velocity).
              if (component_i < dim)
                {
                  const Tensor<1, dim> phi_i_u =
                    fe_values[velocity].value(i, q);
                  cell_rhs(i) += (1.0 / time_step) *
                                 scalar_product(phi_i_u,
