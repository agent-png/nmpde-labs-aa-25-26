...
public:
  ...
  // Constructor.
  DN2D(const unsigned int &subdomain_id_)
  ...
  // Apply Dirichlet conditions on the interface with another DN2D problem.
  void
  apply_interface_dirichlet(const DN2D &other);

  // Apply Neumann conditions on the interface with another DN2D problem.
  void
  apply_interface_neumann(DN2D &other);
...
protected:
  ...
  compute_interface_map(const DN2D &other) const;
