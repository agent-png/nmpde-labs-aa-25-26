...
DN2D::setup()
{
  ...
  {
    fe         = std::make_unique<FE_SimplexP<dim>>(/*r = */1);
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  cell_matrix(i, j) += /*aplha = 1*/ 
                                       fe_values.shape_grad(i, q) *
                                       fe_values.shape_grad(j, q) *
                                       fe_values.JxW(q);
                  cell_matrix(i, j) += /*gamma = 1*/
...
void
DN2D::output(const unsigned int &iter) const
{
...
void
DN2D::apply_interface_dirichlet(const DN2D &other)
{
...
void
DN2D::apply_interface_neumann(DN2D &other)
{
  const auto interface_map = compute_interface_map(other);
...
DN2D::compute_interface_map(const DN2D &other) const
{
...
void
DN2D::apply_relaxation(const Vector<double> &old_solution,
                            const double         &lambda)
{
  solution *= lambda;
