...
public:
  ...
  // Constructor.
  ADR2D(const std::string                               						&mesh_file_name_,
            ...
            const std::function<double(const Point<dim> &)> 				&gamma_)
...
protected:
  ...
  // Advection/Convection (transport) coefficient.
  const std::function<Tensor<1, dim>(const Point<dim> &)> b;
